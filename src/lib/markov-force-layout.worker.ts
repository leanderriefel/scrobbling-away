// Types inlined since workers can't use @/ path aliases

type MarkovGraphNode = {
  id: string;
  label: string;
  plays: number;
...
type ClusterLayoutNode = MarkovGraphNode & {
  x: number;
  y: number;
  radius: number;
  groupId: number;
...
type SimNode = ClusterLayoutNode & { vx: number; vy: number };
type LayoutEdge = { source: SimNode; target: SimNode; weight: number };

type WorkerInput = {
  nodes: MarkovGraphNode[];
  edges: MarkovGraphEdge[];
  membershipEntries: [string, number][];
  groups: VibeGroup[];
};

self.onmessage = (event: MessageEvent<WorkerInput>) => {
  const { nodes, edges, membershipEntries, groups } = event.data;
  const membership = new Map(membershipEntries);
  const result = runForceLayout(nodes, edges, membership, groups);
