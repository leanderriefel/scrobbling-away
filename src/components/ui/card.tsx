...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-none py-4 text-xs/relaxed text-card-foreground has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none",
        variant === "glass" ? "glass-panel noise-overlay" : "bg-card ring-1 ring-foreground/10",
        className,
      )}
      {...props}
    />
